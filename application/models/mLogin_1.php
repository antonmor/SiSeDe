...
class mLogin extends CI_Model{	
	...
	{
	     $var =$this->db->query("select *,p.id as idFisica  from 
			(persona p join RolesPeriodo rp on rp.idPersona=p.id 
			and rp.status=1
            and (idRoles = 8))join Rol r on r.idroles=rp.idroles  
	...
	$arr=$this->db->query("select *, pd.Nombre as 'Demandante', pd.Apat as 'ApatD', pd.Amat as 'AmatD', pd.CURP as 'curpD' , pdo.RazonSocial as 'Demandado'
		from Expediente e 
		join AnexoPDF a on e.id = a.id_Expediente
